...
ggplot(freq.p, mapping = aes(x=Var1, y=Freq))+
  ...
  ggtitle(paste0("number of cell = ",sum(freq.p$Freq),"\n",
                 "number of sample = ", nrow(freq.p)))
ggsave("14.sample_counts_subanalysis_for_DEG_analysis_mild2_progression_and_nonprogression_single_cell_timepoint1/1.Barplot_of_sample_counts_in_Progression_Mild2_CD4_Tcm_after_downsampling.png", device = "png")
...
ggplot(freq.p, mapping = aes(x=Var1, y=Freq))+
  ...
  ggtitle(paste0("number of cell = ",sum(freq.p$Freq),"\n",
                 "number of sample = ", nrow(freq.p)))
ggsave("14.sample_counts_subanalysis_for_DEG_analysis_mild2_progression_and_nonprogression_single_cell_timepoint1/1.Barplot_of_sample_counts_in_Non_Progression_Mild2_CD14_Mono_after_downsampling.png", device = "png")
...
write.table(de.ij, paste0("14.sample_counts_subanalysis_for_DEG_analysis_mild2_progression_and_nonprogression_single_cell_timepoint1//14.Single_cell_DEG_between_Mild1_progression_and_nonprogression_in_CD14_Mono.txt"), 
...
go <- read.delim("14.sample_counts_subanalysis_for_DEG_analysis_mild2_progression_and_nonprogression_single_cell_timepoint1/GO_of_upregulated_genes_in_CD14_Mono_mild1_progression_vs_non_progression/CUSTOM140409815746912.human.enrichr.reports.txt",
                 sep = "\t", header = TRUE)
...
# progressor
logi.df <- read.table("../new_RCA_whole/logitudinal_information_all/logitudinal_sample_with_mild_developing_into_more_severe_for_DEG.txt", 
                      header = TRUE, sep="\t")
logi.df$sampleBatch <- paste(logi.df$BATCH,logi.df$Genotyping_name, sep = "_")
logi.df <- logi.df[(logi.df$type == "rising systemic inflammation"),]
logi.df <- logi.df[(logi.df$sampleBatch %in% all.df$sampleBatch),]
