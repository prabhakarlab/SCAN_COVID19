...
for (cell.a in  seq_along(unique_celltype)){
  ...
  # barplot for non-progression mild1
...
## downsampling DEG
# CD14 Mono
dev.df.sub <- dev.df[(dev.df$cell.annotation == "CD14 Mono"),]
all.df.mild2.sub <- all.df.mild2[(all.df.mild2$cell.annotation == "CD14 Mono"),]
...
for (i in seq_along(sample.down)){
  ...
  down.cell.i <- sample(rownames(dev.df.sub.i), 120)
...
ggplot(freq.p, mapping = aes(x=Var1, y=Freq))+
  ...
  ggtitle(paste0("number of cell = ",sum(freq.p$Freq),"\n",
                 "number of sample = ", nrow(freq.p)))
ggsave("14.sample_counts_subanalysis_for_DEG_analysis_mild1_progression_and_nonprogression_single_cell_timepoint1/1.Barplot_of_sample_counts_in_Progression_Mild1_CD4_Tcm_after_downsampling.png", device = "png")
...
ggplot(freq.p, mapping = aes(x=Var1, y=Freq))+
  ...
  ggtitle(paste0("number of cell = ",sum(freq.p$Freq),"\n",
                 "number of sample = ", nrow(freq.p)))
ggsave("14.sample_counts_subanalysis_for_DEG_analysis_mild1_progression_and_nonprogression_single_cell_timepoint1/1.Barplot_of_sample_counts_in_Non_Progression_Mild1_CD14_Mono_after_downsampling.png", device = "png")
...
write.table(de.ij, paste0("14.sample_counts_subanalysis_for_DEG_analysis_mild1_progression_and_nonprogression_single_cell_timepoint1//14.Single_cell_DEG_between_Mild1_progression_and_nonprogression_in_CD14_Mono.txt"), 
...
# CD16 Mono
dev.df.sub <- dev.df[(dev.df$cell.annotation == "CD16 Mono"),]
all.df.mild2.sub <- all.df.mild2[(all.df.mild2$cell.annotation == "CD16 Mono"),]
dev.df.sub.table <- data.frame(table(dev.df.sub$sampleBatch))
...
ggplot(freq.p, mapping = aes(x=Var1, y=Freq))+
  ...
  ggtitle(paste0("number of cell = ",sum(freq.p$Freq),"\n",
                 "number of sample = ", nrow(freq.p)))
ggsave("14.sample_counts_subanalysis_for_DEG_analysis_mild1_progression_and_nonprogression_single_cell_timepoint1//1.Barplot_of_sample_counts_in_Progression_Mild1_CD16_Mono_after_downsampling.png", device = "png")
...
ggplot(freq.p, mapping = aes(x=Var1, y=Freq))+
  ...
  ggtitle(paste0("number of cell = ",sum(freq.p$Freq),"\n",
                 "number of sample = ", nrow(freq.p)))
ggsave("14.sample_counts_subanalysis_for_DEG_analysis_mild1_progression_and_nonprogression_single_cell_timepoint1/1.Barplot_of_sample_counts_in_Non_Progression_Mild1_CD16_Mono_after_downsampling.png", device = "png")
...
de.ij <- de.ij[grep(pattern = "^MT-|^ERCC|^RPS|^RPL|^HSP", 
                    x = de.ij$gene, invert = TRUE),]
write.table(de.ij, paste0("14.sample_counts_subanalysis_for_DEG_analysis_mild1_progression_and_nonprogression_single_cell_timepoint1//14.Single_cell_DEG_between_Mild1_progression_and_nonprogression_in_CD16_Mono.txt"), 
            sep = "\t", col.names = TRUE,row.names = FALSE, quote = FALSE)
...
for (i in seq_along(cell)){
  ...
  ggplot()+
    ...
    geom_label_repel(data=de.ij.sig[which(de.ij.sig$avg_logFC< -0.35),], 
                     ...
                     point.padding = 0.3,
                     segment.color = 'blue',
                     size=8, color="blue", max.overlaps =100)+
    theme(panel.grid.major = element_blank(), 
          panel.grid.minor = element_blank(),
          panel.background = element_blank(), 
          axis.line = element_line(colour = "black"))
