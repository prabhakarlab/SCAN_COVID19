...
write.table(de.ij, paste0("14.Next_stage_of_mild1_progression_early/14.Single_cell_DEG_between_Mild1_progression_and_NEXT_in_CD14_Mono.txt"), 
...
dev.df.sub.table <- data.frame(table(dev.df.sub$sampleBatch))
sample.down <- as.character(dev.df.sub.table[(dev.df.sub.table$Freq > 80),"Var1"])
for (i in seq_along(sample.down)){
  sample.down.i <- sample.down[i]
  dev.df.sub.i <- dev.df.sub[(dev.df.sub$sampleBatch == sample.down.i),]
...
ggplot(freq.p, mapping = aes(x=Var1, y=Freq))+
  ...
  ggtitle(paste0("number of cell = ",sum(freq.p$Freq),"\n",
                 "number of sample = ", nrow(freq.p)))
ggsave("14.Next_stage_of_mild1_progression_early/1.Barplot_of_sample_counts_in_Progression_Mild1_CD16_Mono_after_downsampling.png", device = "png")
...
ggplot(freq.p, mapping = aes(x=Var1, y=Freq))+
  ...
  ggtitle(paste0("number of cell = ",sum(freq.p$Freq),"\n",
                 "number of sample = ", nrow(freq.p)))
ggsave("14.Next_stage_of_mild1_progression_early/1.Barplot_of_sample_counts_in_Progression_Mild1_NEXT_CD16_Mono_after_downsampling.png", device = "png")
...
de.ij <- de.ij[grep(pattern = "^MT-|^ERCC|^RPS|^RPL|^HSP", 
                    x = de.ij$gene, invert = TRUE),]
write.table(de.ij, paste0("14.Next_stage_of_mild1_progression_early/14.Single_cell_DEG_between_Mild1_progression_and_NEXT_in_CD16_Mono.txt"), 
            sep = "\t", col.names = TRUE,row.names = FALSE, quote = FALSE)
