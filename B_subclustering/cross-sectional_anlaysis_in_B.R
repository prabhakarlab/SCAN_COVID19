...
######
###### marker genes in cell state enriched with severity
######
### Memory B
umap.df.enrich.cd4tcm <- umap.df.enrich[(umap.df.enrich$cell.annotation == "Memory B"),]
...
write.table(de.ij.sig[which(de.ij.sig$avg_logFC > 0),"gene"],
            "12.2D_comparative_enrichment_plot_downsampling/DEG_analysis/upregulated_genes_in_Memory_B_increase_vs_non-increase.txt",
            col.names = FALSE, quote = FALSE, row.names = FALSE)
write.table(de.ij.sig[which(de.ij.sig$avg_logFC < 0),"gene"],
            "12.2D_comparative_enrichment_plot_downsampling/DEG_analysis/downregulated_genes_in_Memory_B_increase_vs_non-increase.txt",
            col.names = FALSE, quote = FALSE, row.names = FALSE)
write.table(de.ij,
            "12.2D_comparative_enrichment_plot_downsampling/DEG_analysis/DEG_in_Memory_B_increase_vs_non-increase.txt",
...
ggplot()+
  ...
  geom_label_repel(data=de.ij.sig[which(de.ij.sig$avg_logFC> .85),], 
  ...
  theme(panel.grid.major = element_blank(), 
        panel.grid.minor = element_blank(),
        panel.background = element_blank(), 
        axis.line = element_line(colour = "black"))+ylim(c(0,650))
ggsave("12.2D_comparative_enrichment_plot_downsampling/DEG_analysis/volcano_plot_of_DEGs_in_Memory_B_increase_vs_non-increase.pdf",
...
for (cell in unique_cell){
  ...
  ggplot()+
    ...
    geom_point(de.ij.sig[which(de.ij.sig$avg_logFC<0),],
               mapping = aes(x = avg_logFC, y = logP), color="blue")+
    geom_label_repel(data=de.ij.sig[which(de.ij.sig$avg_logFC> 1),], 
                     aes(x = avg_logFC, y = logP,label=gene),
                     box.padding   = 1, 
                     point.padding = 0.6,
                     segment.color = 'red',
...
# marker genes of cell states enriched with severity in memory B
go <- read.delim("12.2D_comparative_enrichment_plot_downsampling/DEG_analysis/GO_of_upregulated_genes_in_Memory_B_increase_vs_non-increase/CUSTOM139642269542192.human.enrichr.reports.txt",
                 header = TRUE, sep = "\t")
...
umap.df.sub <- umap.df[(umap.df$cell.annotation == "Memory B"),]
...
ggsave("12.2D_comparative_enrichment_plot_downsampling/DEG_analysis/UMAP_of_heatmap_showing_neutrophil_genes_in_B.png",
         width = 5, height = 4)

# marker genes of cell states enriched with severity in naive B
go <- read.delim("12.2D_comparative_enrichment_plot_downsampling/DEG_analysis/GO_of_upregulated_genes_in_Naive_B_decrease_vs_non-bridge/CUSTOM139642269585488.human.enrichr.reports.txt",
                 header = TRUE, sep = "\t")
go <- go[order(go$Adjusted.P.value, decreasing = FALSE),]
go <- go[1:2,]
gene.1 <- unlist(strsplit(go$Genes[1], split = ";"))
...
umap.df.sub <- umap.df[(umap.df$cell.annotation == "Naive B"),]
...
ggplot()+
  ...
  theme(panel.grid.major = element_blank(), 
        ...
        axis.line = element_line(colour = "black"))+
  scale_colour_gradient2(low = "blue",high = "red",mid = "grey", limits=c(-2,2))
ggsave("12.2D_comparative_enrichment_plot_downsampling/DEG_analysis/UMAP_of_heatmap_showing_interferon_genes_in_B.png",
         width = 5, height = 4)
