...
############
########### enrichment plot between progressors and non-progressors
###########
...
#############
#### DEG between mild1 progressors and non-progressors
############
...
# get mild2 developing into more severe condition
...
## profile cell count for each sample in progression and non-progression mild1
...
ggplot(freq.p, mapping = aes(x=Var1, y=Freq))+
  ...
  ggtitle(paste0("number of cell = ",sum(freq.p$Freq),"\n",
                 "number of sample = ", nrow(freq.p)))
ggsave("14.sample_counts_subanalysis_for_DEG_analysis_mild1_progression_and_nonprogression_single_cell_timepoint1/1.Barplot_of_sample_counts_in_Progression_Mild1_Memory_B_after_downsampling.png", device = "png")


all.df.mild2.sub.table <- data.frame(table(all.df.mild2.sub$sampleBatch))
sample.down <- as.character(all.df.mild2.sub.table[(all.df.mild2.sub.table$Freq > 40),"Var1"])
for (i in seq_along(sample.down)){
  sample.down.i <- sample.down[i]
  all.df.mild2.sub.i <- all.df.mild2.sub[(all.df.mild2.sub$sampleBatch == sample.down.i),]
  down.cell.i <- sample(rownames(all.df.mild2.sub.i), 40)
...
ggplot(freq.p, mapping = aes(x=Var1, y=Freq))+
  ...
  ggtitle(paste0("number of cell = ",sum(freq.p$Freq),"\n",
                 "number of sample = ", nrow(freq.p)))
ggsave("14.sample_counts_subanalysis_for_DEG_analysis_mild1_progression_and_nonprogression_single_cell_timepoint1/1.Barplot_of_sample_counts_in_NonProgression_Mild1_Memory_B_after_downsampling.png", device = "png")
...
write.table(de.ij, paste0("14.sample_counts_subanalysis_for_DEG_analysis_mild1_progression_and_nonprogression_single_cell_timepoint1//14.Single_cell_DEG_between_Mild1_progression_and_nonprogression_in_Memory_B.txt"), 
...
for (i in seq_along(sample.down)){
  sample.down.i <- sample.down[i]
  dev.df.sub.i <- dev.df.sub[(dev.df.sub$sampleBatch == sample.down.i),]
  down.cell.i <- sample(rownames(dev.df.sub.i), 90)
  dev.df.sub <- rbind(dev.df.sub[(dev.df.sub$sampleBatch != sample.down.i),],
                      dev.df.sub.i[down.cell.i,])
}
...
ggplot(freq.p, mapping = aes(x=Var1, y=Freq))+
  ...
  ggtitle(paste0("number of cell = ",sum(freq.p$Freq),"\n",
                 "number of sample = ", nrow(freq.p)))
ggsave("14.sample_counts_subanalysis_for_DEG_analysis_mild1_progression_and_nonprogression_single_cell_timepoint1//1.Barplot_of_sample_counts_in_Progression_Mild1_Naive_B_after_downsampling.png", device = "png")
...
ggplot(freq.p, mapping = aes(x=Var1, y=Freq))+
  ...
  ggtitle(paste0("number of cell = ",sum(freq.p$Freq),"\n",
                 "number of sample = ", nrow(freq.p)))
ggsave("14.sample_counts_subanalysis_for_DEG_analysis_mild1_progression_and_nonprogression_single_cell_timepoint1/1.Barplot_of_sample_counts_in_NonProgression_Mild1_Naive_B_after_downsampling.png", device = "png")
...
de.ij <- de.ij[grep(pattern = "^MT-|^ERCC|^RPS|^RPL|^HSP", 
                    x = de.ij$gene, invert = TRUE),]
write.table(de.ij, paste0("14.sample_counts_subanalysis_for_DEG_analysis_mild1_progression_and_nonprogression_single_cell_timepoint1//14.Single_cell_DEG_between_Mild1_progression_and_nonprogression_in_Naive_B.txt"), 
            sep = "\t", col.names = TRUE,row.names = FALSE, quote = FALSE)
...
################# gene heatmap for upregulated IFN genes in progressors compared to non pregressors
...
go <- read.delim("14.sample_counts_subanalysis_for_DEG_analysis_mild1_progression_and_nonprogression_single_cell_timepoint1/GO_of_union_of_upregulated_genes_across_different_cells/CUSTOM139672779245680.human.enrichr.reports.txt",
                  sep = "\t", header = TRUE)
...
pdf("14.sample_counts_subanalysis_for_DEG_analysis_mild1_progression_and_nonprogression_single_cell_timepoint1//heatmap_of_upregulated_genes_in_mild1_progression_and_non_progression_different_timepoints.pdf")
