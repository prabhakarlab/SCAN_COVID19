...
ggplot()+
  ...
  geom_point(dev.df, mapping = aes(x=UMAP_1, y=UMAP_2), color="#00AE46", size=1, stroke=0)+
...
ggplot(freq.p, mapping = aes(x=Var1, y=Freq))+
  ...
  ggtitle(paste0("number of cell = ",sum(freq.p$Freq),"\n",
                 "number of sample = ", nrow(freq.p)))
ggsave("14.Next_stage_of_mild1_progression_early/1.Barplot_of_sample_counts_in_Progression_Mild1_Memory_B_after_downsampling.png", device = "png")
...
ggplot(freq.p, mapping = aes(x=Var1, y=Freq))+
  ...
  ggtitle(paste0("number of cell = ",sum(freq.p$Freq),"\n",
                 "number of sample = ", nrow(freq.p)))
ggsave("14.Next_stage_of_mild1_progression_early/1.Barplot_of_sample_counts_in_Progression_Mild1_NEXT_Memory_B_after_downsampling.png", device = "png")
...
write.table(de.ij, paste0("14.Next_stage_of_mild1_progression_early/14.Single_cell_DEG_between_Mild1_progression_and_NEXT_in_Memory_B.txt"), 
...
for (i in seq_along(sample.down)){
  sample.down.i <- sample.down[i]
  dev.df.sub.i <- dev.df.sub[(dev.df.sub$sampleBatch == sample.down.i),]
  down.cell.i <- sample(rownames(dev.df.sub.i), 80)
  dev.df.sub <- rbind(dev.df.sub[(dev.df.sub$sampleBatch != sample.down.i),],
                      dev.df.sub.i[down.cell.i,])
}
...
ggplot(freq.p, mapping = aes(x=Var1, y=Freq))+
  ...
  ggtitle(paste0("number of cell = ",sum(freq.p$Freq),"\n",
                 "number of sample = ", nrow(freq.p)))
ggsave("14.Next_stage_of_mild1_progression_early/1.Barplot_of_sample_counts_in_Progression_Mild1_Naive_B_after_downsampling.png", device = "png")
...
ggplot(freq.p, mapping = aes(x=Var1, y=Freq))+
  ...
  ggtitle(paste0("number of cell = ",sum(freq.p$Freq),"\n",
                 "number of sample = ", nrow(freq.p)))
ggsave("14.Next_stage_of_mild1_progression_early/1.Barplot_of_sample_counts_in_Progression_Mild1_NEXT_Naive_B_after_downsampling.png", device = "png")
...
de.ij <- de.ij[grep(pattern = "^MT-|^ERCC|^RPS|^RPL|^HSP", 
                    x = de.ij$gene, invert = TRUE),]
write.table(de.ij, paste0("14.Next_stage_of_mild1_progression_early/14.Single_cell_DEG_between_Mild1_progression_and_NEXT_in_Naive_B.txt"), 
            sep = "\t", col.names = TRUE,row.names = FALSE, quote = FALSE)
