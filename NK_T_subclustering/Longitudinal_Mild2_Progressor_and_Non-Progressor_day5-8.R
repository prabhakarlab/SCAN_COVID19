...
############
########### enrichment plot between Mild2 progressor and non-progressor
###########
...
# mild2 progressor
logi.df <- read.table("../new_RCA_whole/logitudinal_information_all/Mild2_progression", header = TRUE, sep="\t")

logi.df$sampleBatch <- paste(logi.df$BATCH,logi.df$Genotyping_name, sep = "_")
logi.df <- logi.df[(logi.df$sampleBatch %in% all.df$sampleBatch),]

# only keep rising stage
logi.df <- logi.df[(logi.df$type == "rising systemic inflammation"),]
dev.df <- all.df[(all.df$sampleBatch %in% logi.df$sampleBatch),]

# mild2 non-progressor
mild2.nop <- read.table("../new_RCA_whole/logitudinal_information_all/Mild2_nonprogression_rising_stage.txt", 
                        sep="\t",header = TRUE)
# remove redundant samples in donors
mild2.nop <- mild2.nop[!(mild2.nop$sampleBatch %in% c("12_CG23","11_CG25","9_CG28","8_CG45","5_CG65")),]

all.df.mild2 <- all.df[(all.df$sampleBatch %in% mild2.nop$sampleBatch),]

### plot non-progression as ref
ref_num <- nrow(all.df.mild2)
# calculate total number of ref cells in each cell's 300 neighbours
graph_nnMat.ref <- graph_nnMat[,rownames(all.df.mild2)]
graph_nnMat.ref.sum <- Matrix::rowSums(graph_nnMat.ref)

# calculate number of ref and comp cells in each cell's 300 neighbours
graph_nnMat.comp <- graph_nnMat[,rownames(dev.df)]
...
ggsave(paste0("14.sample_counts_subanalysis_for_DEG_analysis_mild2_progression_and_nonprogression_single_cell_timepoint1/UMAP_of_enrichment_ratio_between_progression_and_nonprogression.png")
         ,device = "png",
         width = 7, height = 6)

########################################################################
################## DEG between mild2 progressor and non-progressor
########################################################################
all.df <- FetchData(PBMC.integrated.good, vars = c("sampleBatch","batch","severity",
                                                   "cell.annotation","age"))

# Mild2 progressor
logi.df <- read.table("../new_RCA_whole/logitudinal_information_all/Mild2_progression.txt", header = TRUE, sep="\t")
...
## profile cell count for each sample in progression and non-progression mild2 to check if any sample is overrepresented
for (cell.a in  seq_along(unique_celltype)){
  ...
  ggplot(freq.n, mapping = aes(x=Var1, y=Freq))+
    ...
    ggsave(paste0("14.sample_counts_subanalysis_for_DEG_analysis_mild2_progression_and_nonprogression_single_cell_timepoint1/1.Barplot_of_sample_counts_in_NonProgression_Mild2_",
  ...
  ggplot(freq.p, mapping = aes(x=Var1, y=Freq))+
    ...
    ggsave(paste0("14.sample_counts_subanalysis_for_DEG_analysis_mild2_progression_and_nonprogression_single_cell_timepoint1/1.Barplot_of_sample_counts_in_Progression_Mild2_",
                  "in_",gsub(" ","_",cell),".png"), device = "png")
...
ggplot(freq.p, mapping = aes(x=Var1, y=Freq))+
  ...
  ggtitle(paste0("number of cell = ",sum(freq.p$Freq),"\n",
                 "number of sample = ", nrow(freq.p)))
ggsave(paste0("14.sample_counts_subanalysis_for_DEG_analysis_mild2_progression_and_nonprogression_single_cell_timepoint1/1.Barplot_of_sample_counts_in_Progression_Mild2_CD4_Tcm_after_downsampling.png"), device = "png")
...
write.table(de.ij, paste0("14.sample_counts_subanalysis_for_DEG_analysis_mild2_progression_and_nonprogression_single_cell_timepoint1/14.Single_cell_DEG_between_Mild2_progression_and_nonprogression_in_CD4_Tcm.txt"), 
...
dev.df.sub.table <- data.frame(table(dev.df.sub$sampleBatch))
sample.down <- as.character(dev.df.sub.table[(dev.df.sub.table$Freq > 75),"Var1"])
for (i in seq_along(sample.down)){
  sample.down.i <- sample.down[i]
  dev.df.sub.i <- dev.df.sub[(dev.df.sub$sampleBatch == sample.down.i),]
  down.cell.i <- sample(rownames(dev.df.sub.i), 75)
  dev.df.sub <- rbind(dev.df.sub[(dev.df.sub$sampleBatch != sample.down.i),],
                      dev.df.sub.i[down.cell.i,])
}
...
ggplot(freq.p, mapping = aes(x=Var1, y=Freq))+
  ...
  ggtitle(paste0("number of cell = ",sum(freq.p$Freq),"\n",
                 "number of sample = ", nrow(freq.p)))
ggsave(paste0("14.sample_counts_subanalysis_for_DEG_analysis_mild2_progression_and_nonprogression_single_cell_timepoint1/1.Barplot_of_sample_counts_in_Progression_Mild2_CD4_Tem_after_downsampling.png"), device = "png")
...
ggplot(freq.p, mapping = aes(x=Var1, y=Freq))+
  ...
  ggtitle(paste0("number of cell = ",sum(freq.p$Freq),"\n",
                 "number of sample = ", nrow(freq.p)))
ggsave(paste0("14.sample_counts_subanalysis_for_DEG_analysis_mild2_progression_and_nonprogression_single_cell_timepoint1/1.Barplot_of_sample_counts_in_NonProgression_Mild2_CD4_Tem_after_downsampling.png"), device = "png")
...
de.ij <- de.ij[grep(pattern = "^MT-|^ERCC|^RPS|^RPL|^HSP", 
                    x = de.ij$gene, invert = TRUE),]
write.table(de.ij, paste0("14.sample_counts_subanalysis_for_DEG_analysis_mild2_progression_and_nonprogression_single_cell_timepoint1/14.Single_cell_DEG_between_Mild2_progression_and_nonprogression_in_CD4_Tem.txt"), 
            sep = "\t", col.names = TRUE,row.names = FALSE, quote = FALSE)
...
# CD8 Tem
dev.df.sub <- dev.df[(dev.df$cell.annotation == "CD8 Tem"),]
all.df.mild2.sub <- all.df.mild2[(all.df.mild2$cell.annotation == "CD8 Tem"),]
dev.df.sub.table <- data.frame(table(dev.df.sub$sampleBatch))
sample.down <- as.character(dev.df.sub.table[(dev.df.sub.table$Freq > 40),"Var1"])
for (i in seq_along(sample.down)){
  sample.down.i <- sample.down[i]
  dev.df.sub.i <- dev.df.sub[(dev.df.sub$sampleBatch == sample.down.i),]
  down.cell.i <- sample(rownames(dev.df.sub.i), 40)
  dev.df.sub <- rbind(dev.df.sub[(dev.df.sub$sampleBatch != sample.down.i),],
                      dev.df.sub.i[down.cell.i,])
}
...
ggplot(freq.p, mapping = aes(x=Var1, y=Freq))+
  ...
  ggtitle(paste0("number of cell = ",sum(freq.p$Freq),"\n",
                 "number of sample = ", nrow(freq.p)))
ggsave(paste0("14.sample_counts_subanalysis_for_DEG_analysis_mild2_progression_and_nonprogression_single_cell_timepoint1/1.Barplot_of_sample_counts_in_Progression_Mild2_CD8_Tem_after_downsampling.png"), device = "png")
...
ggplot(freq.p, mapping = aes(x=Var1, y=Freq))+
  ...
  ggtitle(paste0("number of cell = ",sum(freq.p$Freq),"\n",
                 "number of sample = ", nrow(freq.p)))
ggsave(paste0("14.sample_counts_subanalysis_for_DEG_analysis_mild2_progression_and_nonprogression_single_cell_timepoint1/1.Barplot_of_sample_counts_in_NonProgression_Mild2_CD8_Tem_after_downsampling.png"), device = "png")
...
de.ij <- de.ij[grep(pattern = "^MT-|^ERCC|^RPS|^RPL|^HSP", 
                    x = de.ij$gene, invert = TRUE),]
write.table(de.ij, paste0("14.sample_counts_subanalysis_for_DEG_analysis_mild2_progression_and_nonprogression_single_cell_timepoint1//14.Single_cell_DEG_between_Mild2_progression_and_nonprogression_in_CD8_Tem.txt"), 
            sep = "\t", col.names = TRUE,row.names = FALSE, quote = FALSE)
...
ggplot(freq.p, mapping = aes(x=Var1, y=Freq))+
  ...
  ggtitle(paste0("number of cell = ",sum(freq.p$Freq),"\n",
                 "number of sample = ", nrow(freq.p)))
ggsave(paste0("14.sample_counts_subanalysis_for_DEG_analysis_mild2_progression_and_nonprogression_single_cell_timepoint1/1.Barplot_of_sample_counts_in_Progression_Mild2_CD16bright_NK_after_downsampling.png"), device = "png")
...
de.ij <- de.ij[grep(pattern = "^MT-|^ERCC|^RPS|^RPL|^HSP", 
                    x = de.ij$gene, invert = TRUE),]
write.table(de.ij, paste0("14.sample_counts_subanalysis_for_DEG_analysis_mild2_progression_and_nonprogression_single_cell_timepoint1/14.Single_cell_DEG_between_Mild2_progression_and_nonprogression_in_CD16bright_NK.txt"), 
            sep = "\t", col.names = TRUE,row.names = FALSE, quote = FALSE)
...
dev.df.sub.table <- data.frame(table(dev.df.sub$sampleBatch))
sample.down <- as.character(dev.df.sub.table[(dev.df.sub.table$Freq > 200),"Var1"])
for (i in seq_along(sample.down)){
  sample.down.i <- sample.down[i]
  dev.df.sub.i <- dev.df.sub[(dev.df.sub$sampleBatch == sample.down.i),]
  down.cell.i <- sample(rownames(dev.df.sub.i), 200)
  dev.df.sub <- rbind(dev.df.sub[(dev.df.sub$sampleBatch != sample.down.i),],
                      dev.df.sub.i[down.cell.i,])
}
...
ggplot(freq.p, mapping = aes(x=Var1, y=Freq))+
  ...
  ggtitle(paste0("number of cell = ",sum(freq.p$Freq),"\n",
                 "number of sample = ", nrow(freq.p)))
ggsave(paste0("14.sample_counts_subanalysis_for_DEG_analysis_mild2_progression_and_nonprogression_single_cell_timepoint1/1.Barplot_of_sample_counts_in_Progression_Mild2_Cytotoxic_CD8T_after_downsampling.png"), device = "png")
...
ggplot(freq.p, mapping = aes(x=Var1, y=Freq))+
  ...
  ggtitle(paste0("number of cell = ",sum(freq.p$Freq),"\n",
                 "number of sample = ", nrow(freq.p)))
ggsave(paste0("14.sample_counts_subanalysis_for_DEG_analysis_mild2_progression_and_nonprogression_single_cell_timepoint1/1.Barplot_of_sample_counts_in_NonProgression_Mild2_Cytotoxic_CD8T_after_downsampling.png"), device = "png")
...
de.ij <- de.ij[grep(pattern = "^MT-|^ERCC|^RPS|^RPL|^HSP", 
                    x = de.ij$gene, invert = TRUE),]
write.table(de.ij, paste0("14.sample_counts_subanalysis_for_DEG_analysis_mild2_progression_and_nonprogression_single_cell_timepoint1/14.Single_cell_DEG_between_Mild2_progression_and_nonprogression_in_Cytotoxic_CD8_T.txt"), 
            sep = "\t", col.names = TRUE,row.names = FALSE, quote = FALSE)
...
# Naive CD8 T
dev.df.sub <- dev.df[(dev.df$cell.annotation == "Naive CD8 T"),]
all.df.mild2.sub <- all.df.mild2[(all.df.mild2$cell.annotation == "Naive CD8 T"),]
dev.df.sub.table <- data.frame(table(dev.df.sub$sampleBatch))
sample.down <- as.character(dev.df.sub.table[(dev.df.sub.table$Freq > 100),"Var1"])
for (i in seq_along(sample.down)){
  sample.down.i <- sample.down[i]
...
ggplot(freq.p, mapping = aes(x=Var1, y=Freq))+
  ...
  ggtitle(paste0("number of cell = ",sum(freq.p$Freq),"\n",
                 "number of sample = ", nrow(freq.p)))
ggsave(paste0("14.sample_counts_subanalysis_for_DEG_analysis_mild2_progression_and_nonprogression_single_cell_timepoint1/1.Barplot_of_sample_counts_in_Progression_Mild2_Naive_CD8_T_after_downsampling.png"), device = "png")
...
de.ij <- de.ij[grep(pattern = "^MT-|^ERCC|^RPS|^RPL|^HSP", 
                    x = de.ij$gene, invert = TRUE),]
write.table(de.ij, paste0("14.sample_counts_subanalysis_for_DEG_analysis_mild2_progression_and_nonprogression_single_cell_timepoint1/14.Single_cell_DEG_between_Mild2_progression_and_nonprogression_in_Naive_CD8_T.txt"), 
            sep = "\t", col.names = TRUE,row.names = FALSE, quote = FALSE)
...
for (i in seq_along(cell)){
  ...
  deg.file <- read.table(paste0("./14.sample_counts_subanalysis_for_DEG_analysis_mild2_progression_and_nonprogression_single_cell_timepoint1//14.Single_cell_DEG_between_Mild1_progression_and_nonprogression_in_",
                                gsub(" ","_", cell.i),".txt"),
                         sep="\t", header = TRUE)
  ...
  write.table(de.ij.sig[which(de.ij.sig$avg_logFC > 0),"gene"],
              paste0("14.sample_counts_subanalysis_for_DEG_analysis_mild2_progression_and_nonprogression_single_cell_timepoint1//upregulated_genes_in_",
                     gsub(" ","_",cell.i),"_mild1_progression_vs_non_progression.txt"),
              col.names = FALSE, quote = FALSE, row.names = FALSE)
  write.table(de.ij.sig[which(de.ij.sig$avg_logFC < 0),"gene"],
              paste0("14.sample_counts_subanalysis_for_DEG_analysis_mild2_progression_and_nonprogression_single_cell_timepoint1//downregulated_genes_in_",
                     gsub(" ","_",cell.i),"_mild1_progression_vs_non_progression.txt"),
              col.names = FALSE, quote = FALSE, row.names = FALSE)
  ...
  ggplot()+
    ...
    theme(panel.grid.major = element_blank(), 
          ...
          panel.background = element_blank(), 
          axis.line = element_line(colour = "black"))
    ggsave(paste0("14.sample_counts_subanalysis_for_DEG_analysis_mild2_progression_and_nonprogression_single_cell_timepoint1//volcano_plot_in_",
...
for (i in seq_along(cell)){
  ...
  gene.i <- read.table(paste0("14.sample_counts_subanalysis_for_DEG_analysis_mild2_progression_and_nonprogression_single_cell_timepoint1//upregulated_genes_in_",
...
write.table(gene.c,"14.sample_counts_subanalysis_for_DEG_analysis_mild2_progression_and_nonprogression_single_cell_timepoint1/union_of_upregulated_genes_across_different_cells.txt",
            col.names = FALSE, row.names = FALSE,quote = FALSE)
go <- read.delim("14.sample_counts_subanalysis_for_DEG_analysis_mild2_progression_and_nonprogression_single_cell_timepoint1/GO_of_union_of_upregulated_genes_across_different_cells/CUSTOM140300330616224.human.enrichr.reports.txt",
                  sep = "\t", header = TRUE)
...
# Mild2 progressor
logi.df <- read.table("../new_RCA_whole/logitudinal_information_all/Mild2_progression.txt", 
                      header = TRUE, sep="\t")
logi.df$sampleBatch <- paste(logi.df$BATCH,logi.df$Genotyping_name, sep = "_")
logi.df <- logi.df[(logi.df$sampleBatch %in% all.df$sampleBatch),]
logi.df <- logi.df[(logi.df$type == "rising systemic inflammation"),]
...
# mild2 nonprog rising stage
mild2.nop2 <- read.table("../new_RCA_whole/logitudinal_information_all/Mild2_nonprogression_rising_stage.txt", 
                         header = TRUE, sep="\t")
# remove redundant samples in donors
mild2.nop2 <- mild2.nop2[!(mild2.nop2$sampleBatch %in% c("12_CG23","11_CG25","9_CG28","8_CG45","5_CG65")),]
...
pdf(paste0("14.sample_counts_subanalysis_for_DEG_analysis_mild2_progression_and_nonprogression_single_cell_timepoint1//heatmap_of_upregulated_genes_in_mild2_progression_and_non_progression_different_timepoints.pdf"))
