...
early.df <- mod.df[(mod.df$type == "early infection"),]
...
ggsave(paste0("14.Different_stage_of_moderate_nonprogression/0.UMAP_with_moderate_early.png"),device = "png",
         width = 7, height = 6)
...
all.stage.full <- c("rising systemic inflammation",
                    "peak COVID-19 hyper-inflammation",
...
for (cell in unique_cell){
  ...
  ggplot(freq.i, mapping = aes(x=Var1, y=Freq))+
    ...
    ggtitle(paste0("number of cell = ",sum(freq.i$Freq),"\n",
                   "number of sample = ", nrow(freq.i)))
    ggsave(paste("14.Different_stage_of_moderate_nonprogression/sample_count_analysis_for_downsampling_in_DEG/2.Barplot_of_sample_counts_in_NOT_INCREASE_",
                 gsub(" ","_",cell),".png"),
           device = "png")
