...
##########  severe different time course
infect.stat <- read.delim("../new_RCA_whole/logitudinal_information_all/infection_status_for_all_sampleBatches.txt",
                          sep = "\t", header = TRUE)
# severe 
infect.stat.sub <- infect.stat[(infect.stat$severity.value == 5),]

# using day 5-8 as reference instead
early.df <- infect.stat.sub[(infect.stat.sub$cat == "rising systemic inflammation"),]
all.df.early <- all.df[(all.df$sampleBatch %in% early.df$sampleBatch),]

ggplot()+
  geom_point(all.df, 
             mapping = aes(x=UMAP_1, y=UMAP_2), 
             size=.7, stroke=0, color="grey")+
  geom_point(all.df.early, 
             mapping = aes(x=UMAP_1, y=UMAP_2), 
             size=.7, stroke=0, color="red")+
  theme(panel.grid.major = element_blank(), 
        panel.grid.minor = element_blank(),
        panel.background = element_blank(), 
        axis.line = element_line(colour = "black"))
ggsave(paste0("14.Different_stage_of_severe/0.UMAP_with_severe_rising.png"),device = "png",
         width = 7, height = 6)
...
for (i in seq_along(all.stage)){
  ...
  stage.df <- infect.stat.sub[(infect.stat.sub$cat == all.stage.full.i),]
  ...
  ggplot()+
      ...
      theme(panel.grid.major = element_blank(), 
            ...
            panel.background = element_blank(), 
            axis.line = element_line(colour = "black"))
      ggsave(paste0("14.Different_stage_of_severe/",i,".2D_comparative_enrichment_plot_of_",
                    all.stage.i,"_vs_rising.png"),device = "png",
             width = 7, height = 6)
    ...
    ggplot()+
      ...
      theme(panel.grid.major = element_blank(), 
            ...
            panel.background = element_blank(), 
            axis.line = element_line(colour = "black"))
      ggsave(paste0("14.Different_stage_of_severe/",i
                    ,".UMAP_with_severe_",all.stage.i,".png"),device = "png",
             width = 7, height = 6)
}
...
saveRDS(cl_wt, "14.Different_stage_of_severe/cl_wt.rds")
cl_wt <- readRDS("14.Different_stage_of_severe/cl_wt.rds")
...
for (i in seq(1,10,1)){
  ...
  pdf(paste0("14.Different_stage_of_severe/heatmap_of_enrichment_ratios_for_mfuzz_cluster_",
             i,".pdf"))
...
ggplot()+
  ...
  ggsave("14.Different_stage_of_severe/integrated_UMAP_highlighting_cells_with_different_enrichment_trends.png",
...
for (cell in unique_cell){
  ...
  ggplot(freq.i, mapping = aes(x=Var1, y=Freq))+
    ...
    ggsave(paste0("14.Different_stage_of_severe/sample_count_analysis_for_downsampling_in_DEG/2.Barplot_of_sample_counts_in_INCREASE_",
                  gsub(" ","_",cell),".png"),
           device = "png")
  ...
  ggplot(freq.i, mapping = aes(x=Var1, y=Freq))+
    ...
    ggtitle(paste0("number of cell = ",sum(freq.i$Freq),"\n",
                   "number of sample = ", nrow(freq.i)))
    ggsave(paste("14.Different_stage_of_severe/sample_count_analysis_for_downsampling_in_DEG/2.Barplot_of_sample_counts_in_NOT_INCREASE_",
                 gsub(" ","_",cell),".png"),
           device = "png")
  ...
  write.table(de.ij.sig[which(de.ij.sig$avg_logFC > 0),],
              paste0("14.Different_stage_of_severe/DEG_analysis/upregulated_genes_in_",
                     gsub(" ","_",cell),"_increase_vs_non-increase.txt"),
              col.names = TRUE, quote = FALSE, row.names = FALSE, sep = "\t")
  write.table(de.ij.sig[which(de.ij.sig$avg_logFC < 0),],
              paste0("14.Different_stage_of_severe/DEG_analysis/downregulated_genes_in_",
                     gsub(" ","_",cell),"_increase_vs_non-increase.txt"),
              col.names = TRUE, quote = FALSE, row.names = FALSE, sep = "\t")
...
for (i in seq_along(unique_cell)){
  ...
  deg.file <- read.table(paste0("./14.Different_stage_of_severe/DEG_analysis/upregulated_genes_in_",
...
write.table(all.gene,
            "14.Different_stage_of_severe/DEG_analysis/union_of_upregulated_genes_in_increase.txt",
...
for (cell in unique_cell){
  ...
  ggplot(freq.i, mapping = aes(x=Var1, y=Freq))+
    ...
    ggtitle(paste0("number of cell = ",sum(freq.i$Freq),"\n",
                   "number of sample = ", nrow(freq.i)))
    ggsave(paste0("14.Different_stage_of_severe/sample_count_analysis_for_downsampling_in_DEG/2.Barplot_of_sample_counts_in_DECREASE_",
                  gsub(" ","_",cell),".png"),
           device = "png")
  ...
  ggplot(freq.i, mapping = aes(x=Var1, y=Freq))+
    ...
    ggtitle(paste0("number of cell = ",sum(freq.i$Freq),"\n",
                   "number of sample = ", nrow(freq.i)))
    ggsave(paste("14.Different_stage_of_severe/sample_count_analysis_for_downsampling_in_DEG/2.Barplot_of_sample_counts_in_NOT_DECREASE_",
                 gsub(" ","_",cell),".png"),
           device = "png")
...
all.gene <- unique(all.gene)
write.table(all.gene,
            "14.Different_stage_of_severe/DEG_analysis/union_of_upregulated_genes_in_decrease.txt",
            sep = "\t", row.names = FALSE, col.names = FALSE, quote = FALSE)
