...
############
########### enrichment plot between progressor and non-progressor
###########
...
### non-progression
nonprog <- read.table("../new_RCA_whole/logitudinal_information_all/Mild1_nonprogression_early_stage.txt",
                          header = TRUE, sep="\t")

# remove 12_CG59
nonprog <- nonprog[(nonprog$sampleBatch != "12_CG59"),]
all.df.nonprog <- all.df[(all.df$sampleBatch %in% nonprog$sampleBatch),]

### plot non-progressor as ref
...
ggsave(paste0("14.sample_counts_subanalysis_for_DEG_analysis_mild1_progression_and_nonprogression_single_cell_timepoint1/UMAP_of_enrichment_ratio_between_progression_and_nonprogression.png")
         ,device = "png",
         width = 7, height = 6)


#############
#### DEG between mild progressor and non-progressor
############
all.df <- FetchData(PBMC.integrated.good, vars = c("sampleBatch","batch","severity",
                                                   "cell.annotation","age"))

# Mild1 progressor day 1-4
...
# mild1 non-progressor day 1-4
mild2.nop <- read.delim("../new_RCA_whole/logitudinal_information_all/Mild1_nonprogression_early_stage.txt", 
                              sep="\t",header = TRUE)

# remove 12_CG59
mild2.nop <- mild2.nop[(mild2.nop$sampleBatch != "12_CG59"),]
all.df.mild2 <- all.df[(all.df$sampleBatch %in% mild2.nop$sampleBatch),]


unique_celltype <- unique(all.df$cell.annotation)

## profile cell count for each sample in progressors and non-progressors to see if any sample is overrepresented
...
## DEG downsampling if any sample overrepresented
# CD4 Tcm
...
ggplot(freq.p, mapping = aes(x=Var1, y=Freq))+
  ...
  ggtitle(paste0("number of cell = ",sum(freq.p$Freq),"\n",
                 "number of sample = ", nrow(freq.p)))
ggsave(paste0("14.sample_counts_subanalysis_for_DEG_analysis_mild1_progression_and_nonprogression_single_cell_timepoint1/1.Barplot_of_sample_counts_in_Progression_Mild1_CD4_Tcm_after_downsampling.png"), device = "png")


all.df.mild2.sub.table <- data.frame(table(all.df.mild2.sub$sampleBatch))
sample.down <- as.character(all.df.mild2.sub.table[(all.df.mild2.sub.table$Freq > 100),"Var1"])
for (i in seq_along(sample.down)){
  sample.down.i <- sample.down[i]
  all.df.mild2.sub.i <- all.df.mild2.sub[(all.df.mild2.sub$sampleBatch == sample.down.i),]
  down.cell.i <- sample(rownames(all.df.mild2.sub.i),100)
...
write.table(de.ij, paste0("14.sample_counts_subanalysis_for_DEG_analysis_mild1_progression_and_nonprogression_single_cell_timepoint1//14.Single_cell_DEG_between_Mild1_progression_and_nonprogression_in_CD4_Tcm.txt"), 
...
dev.df.sub.table <- data.frame(table(dev.df.sub$sampleBatch))
sample.down <- as.character(dev.df.sub.table[(dev.df.sub.table$Freq > 130),"Var1"])
for (i in seq_along(sample.down)){
  sample.down.i <- sample.down[i]
  dev.df.sub.i <- dev.df.sub[(dev.df.sub$sampleBatch == sample.down.i),]
  down.cell.i <- sample(rownames(dev.df.sub.i), 130)
  dev.df.sub <- rbind(dev.df.sub[(dev.df.sub$sampleBatch != sample.down.i),],
                      dev.df.sub.i[down.cell.i,])
}
...
de.ij <- de.ij[grep(pattern = "^MT-|^ERCC|^RPS|^RPL|^HSP", 
                    x = de.ij$gene, invert = TRUE),]
write.table(de.ij, paste0("14.sample_counts_subanalysis_for_DEG_analysis_mild1_progression_and_nonprogression_single_cell_timepoint1//14.Single_cell_DEG_between_Mild1_progression_and_nonprogression_in_CD4_Tem.txt"), 
            sep = "\t", col.names = TRUE,row.names = FALSE, quote = FALSE)
...
dev.df.sub.table <- data.frame(table(dev.df.sub$sampleBatch))
sample.down <- as.character(dev.df.sub.table[(dev.df.sub.table$Freq > 250),"Var1"])
for (i in seq_along(sample.down)){
  sample.down.i <- sample.down[i]
  dev.df.sub.i <- dev.df.sub[(dev.df.sub$sampleBatch == sample.down.i),]
  down.cell.i <- sample(rownames(dev.df.sub.i), 250)
  dev.df.sub <- rbind(dev.df.sub[(dev.df.sub$sampleBatch != sample.down.i),],
                      dev.df.sub.i[down.cell.i,])
}
...
ggplot(freq.p, mapping = aes(x=Var1, y=Freq))+
  ...
  ggtitle(paste0("number of cell = ",sum(freq.p$Freq),"\n",
                 "number of sample = ", nrow(freq.p)))
ggsave(paste0("14.sample_counts_subanalysis_for_DEG_analysis_mild1_progression_and_nonprogression_single_cell_timepoint1//1.Barplot_of_sample_counts_in_Progression_Mild1_CD16bright_NK_after_downsampling.png"), device = "png")
...
ggplot(freq.p, mapping = aes(x=Var1, y=Freq))+
  ...
  ggtitle(paste0("number of cell = ",sum(freq.p$Freq),"\n",
                 "number of sample = ", nrow(freq.p)))
ggsave(paste0("14.sample_counts_subanalysis_for_DEG_analysis_mild1_progression_and_nonprogression_single_cell_timepoint1/1.Barplot_of_sample_counts_in_NonProgression_Mild1_CD16bright_NK_after_downsampling.png"), device = "png")
...
de.ij <- de.ij[grep(pattern = "^MT-|^ERCC|^RPS|^RPL|^HSP", 
                    x = de.ij$gene, invert = TRUE),]
write.table(de.ij, paste0("14.sample_counts_subanalysis_for_DEG_analysis_mild1_progression_and_nonprogression_single_cell_timepoint1//14.Single_cell_DEG_between_Mild1_progression_and_nonprogression_in_CD16bright_NK.txt"), 
            sep = "\t", col.names = TRUE,row.names = FALSE, quote = FALSE)
...
ggplot(freq.p, mapping = aes(x=Var1, y=Freq))+
  ...
  ggtitle(paste0("number of cell = ",sum(freq.p$Freq),"\n",
                 "number of sample = ", nrow(freq.p)))
ggsave(paste0("14.sample_counts_subanalysis_for_DEG_analysis_mild1_progression_and_nonprogression_single_cell_timepoint1//1.Barplot_of_sample_counts_in_Progression_Mild1_Cytotoxic_CD8T_after_downsampling.png"), device = "png")
...
ggplot(freq.p, mapping = aes(x=Var1, y=Freq))+
  ...
  ggtitle(paste0("number of cell = ",sum(freq.p$Freq),"\n",
                 "number of sample = ", nrow(freq.p)))
ggsave(paste0("14.sample_counts_subanalysis_for_DEG_analysis_mild1_progression_and_nonprogression_single_cell_timepoint1/1.Barplot_of_sample_counts_in_NonProgression_Mild1_Cytotoxic_CD8T_after_downsampling.png"), device = "png")
...
de.ij <- de.ij[grep(pattern = "^MT-|^ERCC|^RPS|^RPL|^HSP", 
                    x = de.ij$gene, invert = TRUE),]
write.table(de.ij, paste0("14.sample_counts_subanalysis_for_DEG_analysis_mild1_progression_and_nonprogression_single_cell_timepoint1/14.Single_cell_DEG_between_Mild1_progression_and_nonprogression_in_Cytotoxic_CD8_T.txt"), 
            sep = "\t", col.names = TRUE,row.names = FALSE, quote = FALSE)
...
for (i in seq_along(cell)){
  ...
  deg.file <- read.table(paste0("./14.sample_counts_subanalysis_for_DEG_analysis_mild1_progression_and_nonprogression_single_cell_timepoint1//14.Single_cell_DEG_between_Mild1_progression_and_nonprogression_in_",
                                gsub(" ","_", cell.i),".txt"),
                         sep="\t", header = TRUE)
  ...
  write.table(de.ij.sig[which(de.ij.sig$avg_logFC > 0),"gene"],
              paste0("14.sample_counts_subanalysis_for_DEG_analysis_mild1_progression_and_nonprogression_single_cell_timepoint1//upregulated_genes_in_",
                     gsub(" ","_",cell.i),"_mild1_progression_vs_non_progression.txt"),
              col.names = FALSE, quote = FALSE, row.names = FALSE)
  write.table(de.ij.sig[which(de.ij.sig$avg_logFC < 0),"gene"],
              paste0("14.sample_counts_subanalysis_for_DEG_analysis_mild1_progression_and_nonprogression_single_cell_timepoint1//downregulated_genes_in_",
                     gsub(" ","_",cell.i),"_mild1_progression_vs_non_progression.txt"),
              col.names = FALSE, quote = FALSE, row.names = FALSE)
  ...
  ggplot()+
    ...
    theme(panel.grid.major = element_blank(), 
          ...
          panel.background = element_blank(), 
          axis.line = element_line(colour = "black"))
    ggsave(paste0("14.sample_counts_subanalysis_for_DEG_analysis_mild1_progression_and_nonprogression_single_cell_timepoint1//volcano_plot_in_",
                  gsub(" ","_",cell.i),"_mild1_progression_vs_non_progression.pdf"),
           width = 11, height = 6)
  
}

################# gene heatmap for upregulated IFN genes in progressors compared to non progressors
# upregulated genes
gene.c <- c()
cell <- c("CD4 Tcm","CD4 Tem","CD16bright NK","Cytotoxic CD8 T")
for (i in seq_along(cell)){
  ...
  gene.i <- read.table(paste0("14.sample_counts_subanalysis_for_DEG_analysis_mild1_progression_and_nonprogression_single_cell_timepoint1//upregulated_genes_in_",
...
write.table(gene.c,"14.sample_counts_subanalysis_for_DEG_analysis_mild1_progression_and_nonprogression_single_cell_timepoint1/union_of_upregulated_genes_across_different_cells.txt",
            col.names = FALSE, row.names = FALSE,quote = FALSE)
go <- read.delim("14.sample_counts_subanalysis_for_DEG_analysis_mild1_progression_and_nonprogression_single_cell_timepoint1/GO_of_union_of_upregulated_genes_across_different_cells.txt",
                  sep = "\t", header = TRUE)
go <- go[c(1,2),]
...
# mild1 nonprog early stage
mild2.nop1 <- read.table("../new_RCA_whole/logitudinal_information_all/Mild1_nonprogression_early_stage.txt", 
                        header = TRUE, sep = "\t")

# remove 12_CG59
mild2.nop1 <- mild2.nop1[(mild2.nop1$sampleBatch != "12_CG59"),]

all.df.mild2.1 <- all.df[(all.df$sampleBatch %in% mild2.nop1$sampleBatch),]

all.df.mild2.1.table <- as.data.frame(table(all.df.mild2.1$sampleBatch))

mild2.nonpro.sample1 <- as.character(all.df.mild2.1.table[(all.df.mild2.1.table$Freq > 10),"Var1"])

mild2.nonpro.gene1 <- data.frame(matrix(nrow = length(gene.c),
                                       ncol = length(mild2.nonpro.sample1)))
for (a in seq_along(mild2.nonpro.sample1)){
  mild2.nonpro.sample1.a <- mild2.nonpro.sample1[a]
  all.df.mild2.a <- all.df.mild2.1[(all.df.mild2.1$sampleBatch == mild2.nonpro.sample1.a),]
  gene.a <- PBMC.integrated.good@assays$RNA@data[gene.c, rownames(all.df.mild2.a)]
  gene.a.mean <- Matrix::rowMeans(gene.a)
  gene.a.mean <- gene.a.mean[gene.c]
  mild2.nonpro.gene1[,a] <- gene.a.mean
}
colnames(mild2.nonpro.gene1) <- paste("nonprogression_early_",seq(1,length(mild2.nonpro.sample1),1),sep = "" )
rownames(mild2.nonpro.gene1) <- gene.c

mild2.gene.all <- cbind(mild2.pro.gene, mild2.nonpro.gene1)
...
pdf(paste0("14.sample_counts_subanalysis_for_DEG_analysis_mild1_progression_and_nonprogression_single_cell_timepoint1//heatmap_of_upregulated_genes_in_mild1_progression_and_non_progression_different_timepoints.pdf"))
